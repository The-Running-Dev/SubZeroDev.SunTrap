...
- **Envelope duplication.** Caught five times in the engine, which keeps the canonical ledger
  ...
  `docs/docs/design/content-and-systems.md` that the engine already owns. Check the envelope, the campaign,
...
- **Use the template's canonical layout.** `docs/` is the installer-managed Docusaurus
  project and `docs/docs/` is the authored documentation root. The installer does not support
  a custom project directory; do not recreate `documentation/` or compensate with workflow
  copy/delete steps.
- **Categories are deliberate.** `vision/`, `design/`, `product/`, and `delivery/` organize
  source. `docs/sidebar.ts` declares the published navigation and reading order;
  adding a Markdown file without adding its id there makes it invisible.
- **The README is the site home.** `docs/src/pages/index.md` is generated by
  `build/ConvertTo-DocumentationHomepage.ps1`; edit `README.md`, regenerate, and commit it.
  Never hand-edit the generated file. It is rendered at `/` without the documentation
  sidebar. `docs/docs/index.md` is a separate authored landing page for `/docs/`. Preserve
...
- **Pin the documentation template.** CI, Pages deployment, `docs/Dockerfile`, and
  `docs.ps1` use the same immutable container digest. To update it, inspect the desired
  template tag with `docker manifest inspect --verbose`, review the image change, and update
  all four references in one PR. Never replace a digest with a mutable tag.
...
- **The template owns the root-route split.** With `routeBasePath: 'docs'`, the installer
  generates the README home at `docs/src/pages/index.md` and keeps authored content under
  `docs/docs/`. Do not copy the generated page into `/template` in CI or turn the docs landing
  into a second README copy. Keep the README's published links aligned with Docusaurus's
  emitted routes — the Vision document resolves at `/docs/vision/`, not
  `/docs/vision/vision/`.
...
- **Nothing here has been played, built, or tested.** The design is unproven in every
  respect. The MVP in `docs/docs/product/mvp.md` exists precisely to find out which parts
  are wrong.
...
- **`docs/docs/design/content-and-systems.md` is the least validated document.** It fixes
